...
export default function ItemKits() {
    ...
    const handleSubmit = async () => {
        if (!name || !sku || selectedItems.length === 0) {
            toast.error('Identification Error: All fields and component nodes required.');
            return;
        }
        ...
        try {
            if (editingKit) {
                ...
                toast.success('Composite Node Updated: Registry sync complete.');
            } else {
                await addItemKit(kitData);
                toast.success('Bundle Materialized: New composite node created.');
            }
            setIsAddOpen(false);
            resetForm();
        } catch (error) {
            toast.error('Protocol Failure: Registry rejected bundle node.');
        }
    };
    ...
    const handleDelete = async (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (!window.confirm('SECURITY_AUDIT: Deconstruct this composite node? Component links will be severed.')) return;
        try {
            await deleteItemKit(id);
            toast.error('Bundle Deconstructed: Composite node erased.');
        } catch (error) {
            toast.error('Protocol failure during deconstruction.');
        }
    };
            ...
            <div className="bg-white border-b border-slate-100 z-50 sticky top-0">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-24 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Bundle Morphology</h1>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mt-1">Composite Matrix • {itemKits.length} Aggregated Nodes</p>
                        </div>
                    </div>
                        ...
                        <Dialog open={isAddOpen} onOpenChange={(val) => { setIsAddOpen(val); if (!val) resetForm(); }}>
                            <DialogTrigger asChild>
                                <Button className="bg-black text-white rounded-[1.2rem] h-14 px-8 font-black uppercase text-[10px] tracking-widest shadow-xl shadow-black/20 hover:scale-[1.02] active:scale-[0.98] transition-all">
                                    ...
                                    Synthesize New Bundle
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="rounded-[3rem] p-12 max-w-2xl border-none shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-none">
                                <DialogHeader>
                                    <DialogTitle className="text-2xl font-black uppercase tracking-tight text-slate-900">Morphology Config</DialogTitle>
                                    <DialogDescription className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Define composite node parameters and constituent links.</DialogDescription>
                                </DialogHeader>

                                <div className="space-y-8 py-8">
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Entity Identity</Label>
                                            <Input value={name} onChange={e => setName(e.target.value)} className="h-16 bg-slate-50 border-none rounded-[1.2rem] px-6 text-[11px] font-black uppercase focus:ring-2 focus:ring-black" placeholder="BUNDLE_ALPHA" />
                                        </div>
                                        <div className="space-y-4">
                                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Node Hash (SKU)</Label>
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Sector (Category)</Label>
                                        ...
                                        <div className="space-y-4">
                                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Aggregate Price</Label>
                                        ...
                                        <div className="space-y-4 min-h-[100px] mb-8">
                                                ...
                                                return (
                                                    <div key={idx} className="bg-slate-50 p-6 rounded-[1.5rem] border border-slate-100 flex items-center justify-between group/item">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className="text-[11px] font-black uppercase text-slate-900 leading-none mb-1">{product?.name || 'NODE_MISSING'}</p>
                                            ...
                                            {selectedItems.length === 0 && (
                                                <div className="py-12 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-300">
                                                    ...
                                                    <p className="text-[10px] font-black uppercase tracking-widest">Awaiting constituent linkings...</p>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <DialogFooter className="gap-4">
                                    <Button variant="ghost" onClick={() => setIsAddOpen(false)} className="h-16 rounded-[1.2rem] font-black uppercase text-[10px] tracking-widest text-slate-400">Abort Protocol</Button>
                                    <Button onClick={handleSubmit} className="h-16 rounded-[1.2rem] bg-black text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-black/20 px-12">
                                        {editingKit ? 'Sync Update' : 'Materialize Bundle'}
                                    </Button>
                                </DialogFooter>
                            </DialogContent>
                        </Dialog>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-white flex flex-col justify-center">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Composite Population</p>
                        <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{itemKits.length} Nodes</h3>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-white flex flex-col justify-center">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Aggregate Magnitude</p>
                    ...
                    <div className="bg-black rounded-[2.5rem] p-10 text-white shadow-2xl shadow-black/20 flex flex-col justify-center relative overflow-hidden group">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Integrity Matrix</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                            <span className="text-[11px] font-black uppercase tracking-widest text-emerald-400">Morphology Validated</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-[3rem] border border-white shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                    <div className="flex-1 relative group w-full">
                        ...
                        <input
                            type="text"
                            placeholder="IDENTIFY BUNDLE BY NAME OR HASH..."
                            value={searchQuery}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredKits.length > 0 ? (
                        filteredKits.map((kit) => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 mb-10">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <Badge className="bg-indigo-50 text-indigo-600 border-indigo-100 rounded-full px-4 py-1 font-black text-[9px] uppercase tracking-widest shadow-sm">
                                            {kit.items?.length || 0} COMPONENT_NODES
                                        </Badge>
                                    </div>
                                </div>
                                ...
                                <div className="pt-8 border-t border-slate-50 flex items-end justify-between">
                                    <div>
                                        <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1 opacity-50">Composite Magnitue</p>
                        ...
                        <div className="col-span-full py-60 text-center opacity-30 flex flex-col items-center justify-center bg-white rounded-[4rem] border-4 border-dashed border-slate-50">
                            ...
                            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Matrix Empty</h3>
                            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mt-4 px-20 max-w-2xl leading-loose text-center">No composite nodes identified in this sector. Synthesize a new bundle to expand the morphology matrix.</p>
                        </div>
                    )}
                </div>
            </main>
